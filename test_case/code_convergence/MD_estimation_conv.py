import torch
from update_MD_conv import update_MD
import numpy as np
import os
import pdb
import sys
...
def main(act, model_path, model_random, train):
    # settings
    ...
    with torch.no_grad():
        ...
        agg=(finalize(agg[0]),finalize(agg[1]))
        # debugging
        #sum_var_0 = torch.sum(agg[mod][1][1],0)==0
        #var_not0 = agg[mod][0][1]!=0
        #print(sum_var_0* var_not0)
    
        MD=(torch.sum(agg[1][1],0)/(2*agg[0][1])).cpu()
...
#ls= list(MD.keys())
        #nans= torch.isnan(MD[ls[1]])
        #infs= torch.isinf(MD[ls[1]])
        #nan_inf= ~nans * ~infs
        #MD_1=MD[ls[1]][nan_inf]
        #mean_MD = {}
        #for k in MD.keys():
